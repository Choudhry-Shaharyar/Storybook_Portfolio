      ...
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-12 gap-8 z-10">
        ...
        >
          <ComicPanel className="h-full flex flex-col justify-center py-12 md:py-20">
            ...
            <p className="text-xl md:text-2xl font-serif text-gray-800 max-w-2xl leading-relaxed border-l-4 border-blue-600 pl-6 my-4">
          ...
          <ComicPanel variant="accent" className="flex flex-col justify-between">
             <div className="space-y-6">
               {HERO_STATS.map((stat, idx) => (
                 <div key={idx} className="border-b border-blue-400 pb-4 last:border-0 last:pb-0">
                   <h4 className="text-4xl font-black font-heading">{stat.value}</h4>
                   <p className="font-bold text-blue-100 uppercase text-xs tracking-wider">{stat.label}</p>
                   <p className="text-sm text-blue-50 mt-1 font-serif italic">{stat.description}</p>
                 </div>
               ))}
             </div>
          </ComicPanel>
